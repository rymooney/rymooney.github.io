...
The following code simulates how likely a cell lineage is to accumulate at least one mutation after repeated divisions. We begin by defining a mutation probability of 0.001, meaning each cell division carries a 0.1% chance of generating a new mutation that persists in the lineage. We then simulate 10,000 independent lineages, each undergoing up to 50 divisions. The code uses tibble() together with map_dbl() to calculate, for each number of divisions, the proportion of lineages that acquire at least one mutation. Inside the mapping function, rbinom(n_cells, size = n, prob = prob_0) simulates the number of mutations per lineage by treating each division as an independent Bernoulli trial with mutation probability prob_0. The resulting vector contains the number of mutations that occurred in each of the 10,000 lineages. By checking which of these counts are greater than zero and taking their mean, we obtain the fraction of lineages that have experienced at least one mutation after n divisions. The results are then combined into a single data frame with two columns—divisions, representing how many cell divisions each lineage undergoes, and prob_mutated, showing the simulated probability that a lineage contains at least one mutation after that many divisions.
...
results <- tibble(
  divisions = number_of_divisions,
  prob_mutated = map_dbl(
    number_of_divisions,
    function(n) {
      mutations_per_cell <- rbinom(n_cells, size = n, prob = prob_0)
      mutated_flag <- mutations_per_cell > 0
      mean(mutated_flag)
    }
  )
)
